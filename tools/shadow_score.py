#!/usr/bin/env python3
"""
Score pre-registered GPT-5.6 / Fable 5 shadow lanes from existing resolution files.

This does not settle markets itself. Run the existing tools/resolve.py first.
It then reports lane-specific Brier, lane-specific market Brier, Brier Skill Score,
log loss, theoretical P&L, checkpoint status, and paired common-ticker diagnostics.
"""
...
# Historical-reference discipline from the pre-registration.
...
def paired_diagnostics(rows: list[dict], scan_meta: dict) -> dict:
    ...
    for (scan_date, ticker, snap), recs in groups.items():
        ...
        ordered = sorted(m for m in models if m)
